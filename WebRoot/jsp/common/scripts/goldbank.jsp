       ...
       // return;
        ...
        if(rsp=="-1"){
        	...
        	if(policy=="-3" || policy=="-2"){
        		...
        		showModalMessage("金库策略未配置，操作阻止，请联系管理员！");
        		goldBankErrorAudit(operCode,operContent,6,"");
        	}else if(policy=="-5"){//未配置策略阻止
        		showModalMessage("金库策略操作人和审批人相同，操作阻止，请联系管理员！");
       ...
       // return;
        ...
        if(rsp=="1"){
        	...
        	if(policy=="4"){//出现错误或异常（包括数据异常）
        		showModalMessage("金库帐号登录策略查询出现异常，请联系管理员！");
        		goldBankErrorAudit(operCode,operContent,policy,"");
        	}else if(policy=="6"){//未配置策略阻止
        		showModalMessage("金库帐号登录策略未配置审批人，操作阻止，请联系管理员！");
        		goldBankErrorAudit(operCode,operContent,policy,"");
        	}else if(policy=="-4"){//未配置策略阻止
        		showModalMessage("应急状态下，临时金库帐号不允许登录，请联系管理员！");
        		goldBankErrorAudit(operCode,operContent,policy,"");
        	}else if(policy=="-5"){//未配置策略阻止
        		showModalMessage("金库帐号登录审批人不能为当前操作主帐号，请联系管理员！");
        		goldBankErrorAudit(operCode,operContent,policy,"");
        	}else{//非金库帐号登录
        		eval(func);
        	}
			...
			**/
			...
			if(returnType=="-3"){
				...
				showModalMessage("金库审批页面返回值异常，按应急处理!",func);
			}
        }
	}
	...
	function doGoldBankAppoint(isInner,appointType,resKind,resEntityId,operateValue,operContent,func){
		...
		**/
		...
		if(returnType=="-1"){
			...
			showModalMessage("金库审批页面返回值异常，操作中止!");
		}
	}
